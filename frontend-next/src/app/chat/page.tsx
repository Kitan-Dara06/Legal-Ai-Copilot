...
export default function ChatPage() {
        ...
        supabase.auth.getSession().then(
            ({
                ...
                if (authSession) {
                    ...
                    const savedOrg =
                        localStorage.getItem("legalrag_active_org") ||
                        undefined;
        ...
        listWorkspaces(token, user.org_slug)
            .then((workspaces) => {
                if (workspaces && workspaces.length > 0) {
                    ...
                    console.warn(
                        "[ChatPage] No workspaces found, creating default...",
                    );
                    createWorkspace(
                        token,
                        "Default Workspace",
    ...
    const loadWorkspaceData = useCallback(() => {
        ...
        getWorkspace(token, workspaceId, orgSlug)
            .then((ws) => {
                const docs = (ws.documents || []).map(
                    (d: WorkspaceDocument) => ({
                        file_id:
                            parseInt(
                                d.document_id.replace(/-/g, "").substring(0, 8),
                                16,
                            ) || 0,
            ...
            const interval = setInterval(async () => {
                try {
                    ...
                    if (detail.status === "AWAITING_APPROVAL") {
                        // Workflow is waiting for human approval — fetch actions
                        const workflows = (detail as any).workflows as
                            | any[]
                            | undefined;
                        if (workflows && workflows.length > 0) {
                            const wfId = workflows[0].id;
                            try {
                                const actData = await getWorkflowActions(
                                    token,
                                    wfId,
                                    orgSlug,
                                );
                    ...
                    console.error(
                        `[ChatPage] Polling error for goal ${goalId}:`,
                        err,
                    );
                    clearInterval(interval);
                    pollingRef.current.delete(goalId);
                ...
                prev.map((g) =>
                    g.id === tempGoal.id
                        ? {
                              ...
                              intent:
                                  (response.primary_intent as GoalIntent) ||
                                  null,
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-navy-950 via-navy-950/80 to-transparent pointer-events-none">
                        ...
                        {activeSessionId && selectedDocumentIds.length > 0 && (
                            <div className="mt-2 text-xs text-slate-500 text-center">
                                🔍 Searching {selectedDocumentIds.length}{" "}
                                selected document(s)
                                <button
