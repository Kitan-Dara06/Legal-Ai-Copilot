        ...
        return (
            <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-800">
                ...
                <p className="text-xs text-slate-400 mb-4 leading-relaxed">
                    Select documents above to create an isolated RAG session for
                    querying.
                </p>
                <Button
            ...
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                    <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                        <div>
                            Active Session
                            <p className="text-[10px] text-accent-blue font-normal uppercase tracking-wider mt-0.5">
                                Expires in ~{session.ttl_hours}h
                            </p>
                        </div>
                    </h3>
                ...
                <div className="bg-navy-950/50 rounded-lg border border-slate-800 p-2 mb-4">
                    <p className="text-xs font-medium text-slate-300 mb-2 px-1 text-[10px] uppercase tracking-wider">
                        Loaded Documents
                    </p>
                    <ul className="space-y-1 max-h-[120px] overflow-y-auto pr-1">
                        {session.files?.map((f, i) => (
                            <li
                                key={`${f.file_id}-${i}`}
                                className="flex items-center gap-2 text-xs py-1 px-2 rounded bg-slate-900/50"
                            >
                ...
                <div className="flex gap-2">
                    {selectedFileIds.length > 0 &&
                        selectedFileIds.some(
                            (id) =>
                                !session.files?.find((f) => f.file_id === id),
                        ) && (
                            <Button
