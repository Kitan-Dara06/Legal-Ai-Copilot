  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        if (!workspaceId)
          throw new Error("Workspace ID is required for upload");
    ...
    [orgSlug, onUploadSuccess, workspaceId],
  );
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <input {...getInputProps()} />
        <UploadCloud
          className={`w-8 h-8 mx-auto mb-3 ${isDragActive ? "text-accent-blue" : "text-slate-500"}`}
        />
