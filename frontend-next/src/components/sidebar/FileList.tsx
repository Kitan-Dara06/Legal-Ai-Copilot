...
function getDocId(f: FileItem): string {
  return f.document_id || String(f.file_id);
}
          ...
          <ul className="divide-y divide-slate-800/50">
            {paginatedFiles.map((f) => {
              ...
              return (
                <li
                  key={docId}
                  className={`group flex items-center justify-between p-4 hover:bg-slate-800/30 transition-colors cursor-pointer ${isSelected ? "bg-accent-blue/10 hover:bg-accent-blue/20" : ""}`}
                  onClick={() => onToggleSelection(docId)}
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div
                      className={`shrink-0 w-5 h-5 rounded border flex items-center justify-center transition-colors ${isSelected ? "border-accent-blue bg-accent-blue text-white" : "border-slate-600 bg-slate-900/50"}`}
                    >
                      {isSelected && (
                        <svg
      ...
      {totalPages > 1 && (
        <div className="p-3 border-t border-slate-800 flex items-center justify-between bg-slate-900/50">
          <p className="text-xs text-slate-500">
            ...
            {Math.min((page + 1) * ITEMS_PER_PAGE, filteredFiles.length)} of{" "}
