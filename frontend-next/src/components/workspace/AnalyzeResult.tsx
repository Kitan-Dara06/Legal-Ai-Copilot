...
export function AnalyzeResult({ result }: AnalyzeResultProps) {
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-sm">
                            The system could not find enough relevant evidence in the documents to answer confidently.
    ...
    const confidenceColor = result.confidence >= 0.8 ? "green" : result.confidence >= 0.5 ? "yellow" : "red";
    const faithfulnessColor = result.faithfulness_score >= 0.8 ? "green" : result.faithfulness_score >= 0.5 ? "yellow" : "red";
        ...
        <Card className="mb-6 p-4">
            ...
            {result.source_citations && result.source_citations.length > 0 && (
                <div className="mt-4">
                    ...
                    >
                        {expandedSources ? "Hide" : "Show"} sources ({result.source_citations.length})
                    </button>
                    {expandedSources && (
