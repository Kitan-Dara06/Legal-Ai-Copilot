...
export function ReasonResult({ result }: ReasonResultProps) {
    ...
    const hasConflicts = result.definitional_conflicts && result.definitional_conflicts.length > 0;
    const hasEscalations = result.escalations && result.escalations.length > 0;

    const escalationColor = (type: string) => {
        switch (type) {
            case "DEFINITIONAL_CONFLICT": return "yellow";
            case "STRUCTURAL_AMBIGUITY": return "red";
            case "DEGRADED": return "orange";
            case "INSUFFICIENT_COVERAGE": return "red";
        ...
        <div className="space-y-4 mb-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-xs mt-1">
                                Some terms in the context have conflicting definitions across documents.
            ...
            {result.graph_degraded && (
                <Card className="p-4 border-orange-500/30">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-orange-400 font-semibold text-sm">Degraded Mode</h3>
                            <p className="text-slate-400 text-xs mt-1">
                                Cross-reference graph was unavailable. Results are retrieval-only.
                            </p>
                        </div>
                    </div>
                </Card>
            ...
            <Card className="p-4">
                <div className="flex items-center justify-between mb-3">
                    ...
                    {hasEscalations && (
                        <Badge color="red">{result.escalations!.length} escalations</Badge>
                    )}
                </div>
            ...
            {hasConflicts && (
                <Card className="p-4">
                    <h3 className="text-white font-semibold mb-3">Definitional Conflicts</h3>
                    <div className="space-y-2">
                        {result.definitional_conflicts!.map((dc, i) => (
                            <div key={i} className="p-3 bg-slate-800/50 rounded-lg">
                                <div className="flex items-center gap-2 mb-1">
                                    <Badge color="yellow" size="sm">Conflict</Badge>
                                    <span className="text-white text-sm font-medium">{dc.term}</span>
                                </div>
                                <p className="text-slate-400 text-xs">
                                    {dc.conflict_description || `Defined in document ${dc.source_document_id}`}
                                </p>
                            </div>
                        ))}
                    </div>
                ...
                <Card className="p-4">
                    ...
                    <div className="space-y-2">
                        {result.escalations!.map((esc, i) => (
                            <div key={i} className="p-3 bg-slate-800/50 rounded-lg">
                                <div className="flex items-center gap-2 mb-1">
                                    <Badge color={escalationColor(esc.type)} size="sm">{esc.type}</Badge>
                                    {esc.term && (
