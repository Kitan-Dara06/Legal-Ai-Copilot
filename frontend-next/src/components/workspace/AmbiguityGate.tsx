...
export function AmbiguityGate({ detectedIntent, confidence, onConfirm }: AmbiguityGateProps) {
    ...
    return (
        <Card className="mb-6 p-4 border-blue-500/30">
            <div className="space-y-4">
                <div>
                    <h3 className="text-white font-semibold text-lg">Confirm Your Intent</h3>
                    <p className="text-slate-400 text-sm mt-1">
                        I detected your goal as: <span className="text-blue-400 font-medium">{detectedIntent}</span>{" "}
                ...
                <div className="flex items-center gap-3">
                    <Button onClick={() => onConfirm(selectedIntent)}>
                        Confirm
                    </Button>
                ...
                {showOptions && (
                    <div className="space-y-2 pt-2">
                        <p className="text-slate-400 text-xs mb-2">Choose a different intent:</p>
                        {INTENT_OPTIONS.map((intent) => (
                            ...
                            >
                                ...
                                />
                                <div>
                                    <span className="text-white text-sm font-medium">{intent}</span>
                                    <p className="text-slate-400 text-xs">{intentLabels[intent]}</p>
                                </div>
                            </label>
                        ))}
                    </div>
