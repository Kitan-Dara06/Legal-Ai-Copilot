...
from sqlalchemy import (
    ...
    PrimaryKeyConstraint, String, Text, Uuid, Float, UniqueConstraint
)
...
class User(Base):
    ...
    personal_org_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("organizations.id"), nullable=True, index=True)
    full_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))

class UserOrgMembership(Base):
    __tablename__ = "user_org_memberships"
    __table_args__ = (PrimaryKeyConstraint("user_id", "org_id", name="pk_user_org_memberships"),)
...
class Workspace(Base):
    ...
    intelligence_status: Mapped[IntelligenceStatus] = mapped_column(Enum(IntelligenceStatus), nullable=False, default=IntelligenceStatus.PENDING)
...
class Document(Base):
    ...
    file_type: Mapped[DocumentType | None] = mapped_column(Enum(DocumentType), nullable=True)
    ...
    intelligence_stages_complete: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    error_message: Mapped[str | None] = mapped_column(Text, nullable=True)
    upload_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc), index=True)
...
class Goal(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("workspaces.id"), nullable=False, index=True)
    session_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("workspace_sessions.id"), nullable=True, index=True)
    org_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("organizations.id"), nullable=False)
    user_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("users.id"), nullable=False)
    goal_text: Mapped[str] = mapped_column(Text, nullable=False)
    goal_hash: Mapped[str | None] = mapped_column(String(64), nullable=True)
    document_subset: Mapped[list[uuid.UUID] | None] = mapped_column(ARRAY(Uuid), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
...
class WorkflowExecution(Base):
    ...
    org_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("organizations.id"), nullable=False)
    intent: Mapped[IntentType] = mapped_column(Enum(IntentType), nullable=False, default=IntentType.UNKNOWN)
    intent_confidence: Mapped[float | None] = mapped_column(Float, nullable=True)
    intent_reasoning: Mapped[str | None] = mapped_column(Text, nullable=True)
    intent_confirmed_by_human: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    confirmed_intent: Mapped[IntentType | None] = mapped_column(Enum(IntentType, name="confirmed_intent_type"), nullable=True)
    status: Mapped[WorkflowStatus] = mapped_column(Enum(WorkflowStatus), nullable=False, default=WorkflowStatus.CLASSIFYING)
...
class Finding(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    workflow_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("workflow_executions.id"), nullable=False, index=True)
    org_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("organizations.id"), nullable=False)
    claim: Mapped[str] = mapped_column(Text, nullable=False)
    confidence: Mapped[float] = mapped_column(Float, nullable=False)
    supporting_citations: Mapped[list[dict] | None] = mapped_column(JSONB, nullable=True)
    reference_chain: Mapped[list[dict] | None] = mapped_column(JSONB, nullable=True)
    definitional_conflicts: Mapped[list[dict] | None] = mapped_column(JSONB, nullable=True)
    escalated: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    escalation_type: Mapped[EscalationType | None] = mapped_column(Enum(EscalationType), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
...
class Action(Base):
    ...
    status: Mapped[ActionStatus] = mapped_column(Enum(ActionStatus), nullable=False, default=ActionStatus.DETECTED)
    ...
    deadline_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("deadline_registry.id"), nullable=True)
...
class ApprovalRequest(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    workflow_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("workflow_executions.id"), nullable=False, index=True)
    action_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("actions.id"), nullable=False, index=True)
    org_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("organizations.id"), nullable=False)
    ...
    actor: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("users.id"), nullable=True)
...
class AuditLog(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    workflow_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("workflow_executions.id"), nullable=False, index=True)
    action_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("actions.id"), nullable=True)
    org_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("organizations.id"), nullable=False)
...
class DefinedTermRegistry(Base):
    ...
    source_document_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("documents.id"), nullable=False)
...
class DeadlineRegistry(Base):
    ...
    resolution_status: Mapped[DeadlineResolutionStatus] = mapped_column(Enum(DeadlineResolutionStatus), nullable=False, default=DeadlineResolutionStatus.RELATIVE_UNRESOLVED)
    conflict_flag: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    status: Mapped[DeadlineStatus] = mapped_column(Enum(DeadlineStatus), nullable=False, default=DeadlineStatus.ACTIVE)
...
class ToolCallLog(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    workflow_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("workflow_executions.id"), nullable=False, index=True)
    action_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("actions.id"), nullable=False)
    tool_name: Mapped[str] = mapped_column(String(100), nullable=False)
    idempotency_key: Mapped[str] = mapped_column(String(64), unique=True, index=True, nullable=False)
    attempt_number: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    status: Mapped[ToolCallStatus] = mapped_column(Enum(ToolCallStatus), nullable=False, default=ToolCallStatus.PENDING)
