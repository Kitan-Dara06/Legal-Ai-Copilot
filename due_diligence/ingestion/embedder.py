...
class LegalEmbedder:
    def __init__(self, collection_name: str = COLLECTION_NAME, client: QdrantClient = None):
        ...
        if self._voyage_available:
            ...
            print("⚠️  VOYAGE_API_KEY not set — falling back to nomic-embed-text-v1.5 as primary.")

        print(f"Loading nomic backup model ({NOMIC_MODEL})...")
        self._nomic = SentenceTransformer(NOMIC_MODEL, trust_remote_code=True)
        ...
        if not self.qdrant_client.collection_exists(self.collection_name):
            ...
            self.qdrant_client.create_collection(
                ...
                vectors_config={
                    ...
                    "dense_nomic": VectorParams(size=NOMIC_DIM, distance=Distance.COSINE),
                },
    ...
    def _embed_voyage(self, texts: List[str]) -> List[List[float]]:
        """Call Voyage AI with fallback to nomic on any error."""
        ...
        try:
            ...
            print(f"  ⚠️ Voyage embed failed ({e}), falling back to nomic.")
            return self._embed_nomic(texts)

    def _embed_nomic(self, texts: List[str]) -> List[List[float]]:
        # nomic requires "search_document: " prefix for indexing
        prefixed = [f"search_document: {t}" for t in texts]
        vecs = self._nomic.encode(prefixed, normalize_embeddings=True)
    ...
    def index_document(self, document_name: str, chunks: List[Dict]):
        ...
        # Nomic backup stored alongside voyage for RRF fusion
        nomic_vecs = self._embed_nomic(rich_texts)

        points = []
        for i, (chunk, rich_text, v_vec, n_vec) in enumerate(
            zip(chunks, rich_texts, voyage_vecs, nomic_vecs)
        ):
    ...
    def get_voyage_query_vector(self, text: str) -> List[float]:
        """Embed a query with voyage-law-2 (falls back to nomic on failure)."""
        if not self._voyage_available:
            return self.get_nomic_query_vector(text)
        try:
            result = self._voyage.embed([text], model=VOYAGE_MODEL, input_type="query")
            return result.embeddings[0]
        except Exception as e:
            print(f"  ⚠️ Voyage query embed failed ({e}), falling back to nomic.")
            return self.get_nomic_query_vector(text)

    def get_nomic_query_vector(self, text: str) -> List[float]:
        """Embed a query with nomic-embed-text-v1.5."""
        # nomic requires "search_query: " prefix for queries
        prefixed = f"search_query: {text}"
        return self._nomic.encode(prefixed, normalize_embeddings=True).tolist()

    def get_splade_query_vector(self, text: str) -> SparseVector:
        return self._embed_splade(text)

    # ------------------------------------------------------------------
    # Legacy aliases (keep routes.py and hybrid_search.py working)
    # ------------------------------------------------------------------

    def get_bge_query_vector(self, text: str) -> List[float]:
        """Alias: voyage-law-2 is now the primary dense vector."""
        return self.get_voyage_query_vector(text)

    def get_legal_bert_query_vector(self, text: str) -> List[float]:
        """Alias: nomic is now the backup dense vector."""
        return self.get_nomic_query_vector(text)
